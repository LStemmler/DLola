...
public final class RelevantSubgraph {
	...
	public HashSet<Node> getOptions(PathTree pt, SubexpressionTree subext) throws PathGenerationException {
		HashSet<Input> requiredInputs = subext.getRequiredInputs();
		if (!getReachableInputs().containsAll(requiredInputs)) {
			throw new PathGenerationException("Unreachable inputs during getOptions");
		}
		if (subext.bandwidth == 0 || subext.optSubBandwith == 0) {
			HashSet<Node> options = new HashSet<Node>();
			options.add(target);
			return options;
		}
		return getOptions(pt, subext, target);
	}

	private HashSet<Node> getOptions(PathTree pt, SubexpressionTree subext, Node n) {
		HashSet<Input> requiredInputs = subext.getRequiredInputs();
		HashSet<Node> options = new HashSet<Node>();
		if (getReachableInputs(n).containsAll(requiredInputs)) {
			if (subext.input) {
				if (pt.getAvailableSubexTrees(getDLolaNode(n)).containsKey(subext)) {
					// Input in node
					options.add(n);
					return options;
				} else {
				...
				if (pt.getAvailableSubexTrees(getDLolaNode(n)).containsKey(subext)) {
					...
					while (containsChildren) {
						if (index == subext.getSplitList().size()) {
							// contains all children
							return options;
						}
						containsChildren = pt.getAvailableSubexTrees(getDLolaNode(n)).containsKey(subext.getSplitList().get(index++));
					}
				}
			}
			for (Edge e: n.getEnteringEdgeSet()) {
				if (!isIndirect(e)) {
					options.addAll(getOptions(pt, subext, e.getSourceNode()));
				}
			}
		}
		return options;
