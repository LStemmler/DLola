...
public class SystemModel {
	...
	HashMap<ParseTree, SubexpressionTree> SubexpressionTreeMap = new HashMap<ParseTree, SubexpressionTree>(); // Maps
																												// ParseTrees
																												// to
																												// their
																												// equivalent
	...
	private void subgraphAnalysis() {
		...
		SubexpressionTree.generateSubexpressionTrees();
	...
	public HashSet<Input> getRequiredInputs(ParseTree tree) {
		return getSubexpTree(tree).getRequiredInputs();
	}

	public int getBandwidth(ParseTree tree) {
		try {
			return getSubexpTree(tree).getBandwidth();
	...
	public int getOptimalSubtreesBandwidth(ParseTree tree) {
		try {
			return getSubexpTree(tree).getOptSubBandwidth();
		} catch (NullPointerException e) {
			return 0;
		}
	}

	public boolean hasOptimalBandwidth(ParseTree tree) {
		return getSubexpTree(tree).hasOptimalBandwidth();
	}

	public SubexpressionTree getSubexpTree(ParseTree tree) {
		return SubexpressionTreeMap.get(tree);
	}

	public void setSubexpTree(ParseTree tree, SubexpressionTree subextree) {
		SubexpressionTreeMap.put(tree, subextree);
	}

	public void unsetSubexpTree(ParseTree tree) {
		SubexpressionTreeMap.remove(tree);
	}
